...
let LeagueDetail = async (body, req, res) => {
  try {
    ...
    return apiResponse.onSuccess(res, "League detail fetched successfully.", 200, true, result);
  } catch (err) {
    console.log("err ", err);
    return apiResponse.onError(res, "An error occurred while league detail fetching.", 500, false);
  }
};
let LeagueJoinRequest = async (body, req, res) => {
  try {
    ...
    return apiResponse.onSuccess(res, "League join player request fetched successfully.", 200, true, request_list);
  } catch (err) {
    console.log("err ", err);
    return apiResponse.onError(res, "An error occurred while League join player request detail fetching.", 500, false);
  }
};
let LeaguePlayersList = async (body, req, res) => {
  try {
    ...
    return apiResponse.onSuccess(res, "League player fetched successfully.", 200, true, player_list);
  } catch (err) {
    console.log("err ", err);
    return apiResponse.onError(res, "An error occurred while league player fetching.", 500, false);
  }
};
let LeaguePlayersListByRating = async (body, req, res) => {
  try {
    ...
    return apiResponse.onSuccess(res, "League players fetched successfully.", 200, true, player_list);
  } catch (err) {
    console.log("err ", err);
    return apiResponse.onError(res, "An error occurred while fetching league players.", 500, false);
  }
};
...
let ProcessRequest = async (body, req, res) => {
  try {
    ...
    if (!player_id || !mongoose.Types.ObjectId.isValid(player_id)) {
      return apiResponse.onSuccess(res, "Please provide a valid player id.", 400, false);
    }

    // Validate player_id
    if (!status || (status && ![2,3].includes(Number(status)))) {
      return apiResponse.onSuccess(res, "Please provide a valid status.", 400, false);
    }

    // Check if sport exists
    let league_request = await LeaguePlayerModel.findOne({league_id : new mongoose.Types.ObjectId(league_id), player_id: new mongoose.Types.ObjectId(player_id)});
    
    if(!league_request) {
      return apiResponse.onSuccess(res, "Please provide a valid league request data.", 400, false);
    }
    ...
    return apiResponse.onSuccess(res, "Pleayer request"+genmessage+"successfully", 200, true);
  } catch (err) {
    console.log("err ", err);
    return apiResponse.onError(res, "An error occurred while creating the league.", 500, false);
  }
...
let PlayerDetail = async (body, req, res) => {
  try {
    ...
    if (!playerDetails || playerDetails.length === 0) {
      return apiResponse.onSuccess(res, "Selected player not found.", 400, false);
    }

    return apiResponse.onSuccess(res, "Player detail fetched successfully.", 200, true, playerDetails[0]);
  } catch (err) {
    console.log("err ", err);
    return apiResponse.onError(res, "An error occurred while fetching the player.", 500, false);
  }
};
module.exports = {
