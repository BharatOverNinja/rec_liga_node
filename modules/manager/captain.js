...
let ChangeRequestStatus = async (body,req, res) => {
    try {
      ...
      let captain_limt = await CaptainModel.find({event_id : new mongoose.Types.ObjectId(event_id)})
      if(captain_limt.length >= 2) {
        return apiResponse.onSuccess(res, "Another captain has accepted request.", 400, false);
      }
      ...
      if(Number(request_status) == 2) {
        let attended_event_player = await EventAttandanceModel.findOne({event_id: event_id,user_id: user_id })
